...
logger = logging.getLogger("VDB_Service")
...
OLLAMA_EMB_MODEL = os.getenv("OLLAMA_EMB_MODEL") #"qwen3-embedding:0.6b"
MAX_EMB_CHARS = os.getenv("MAX_EMB_CHARS") #8000
...
HF_MODEL_NAME = os.getenv("EMBEDDING_MODEL_NAME") #"intfloat/multilingual-e5-base"

# Initialisation du modèle (une seule fois au démarrage)
custom_embedder = CustomEmbedder(HF_MODEL_NAME)
...
def delete_collection(req: DeleteCollectionRequest):
    """
    Supprime (DROP) une collection Milvus.
    Attention: irréversible (perte totale des données + index).
    """
    try:
        client = MilvusClient(uri=MILVUS_URI)

        if not client.has_collection(req.collection):
            if req.ignore_missing:
                return {"ok": True, "collection": req.collection, "dropped": False, "reason": "missing"}
            raise HTTPException(status_code=404, detail=f"Collection '{req.collection}' introuvable")

        # Optionnel : tenter de décharger avant drop (selon versions / état cluster)
        try:
            client.release_collection(req.collection)
        except Exception:
            pass

        client.drop_collection(req.collection)
        return {"ok": True, "collection": req.collection, "dropped": True}
